...
- **Status**: Stock_Reservation__c FLS added to `Fulfillment_Operations` (unblocked the Flow). Then a dedicated **`Admin_Field_Access`** permission set with FLS for *all* custom fields (48 entries) was deployed + assigned to the admin, so admins see every field in Setup/Flow Builder/manual steps. Required fields omitted (always visible + they *reject* FLS deploys, e.g. `Customer_Price__c.Account__c`); formula fields (`Order_Stage__c`, `Discount_Amount__c`) are read-only in FLS. Per-role FLS on the functional permission sets (end-users see only their fields) is still a tracked follow-up.
