...
- **`Price_Tier__c` link target (resolved during Block D materialization, 2026-05-30)**: originally specified as a single **Lookup to `PricebookEntry`** to make tiers segment-aware. **Materialization proved this is not buildable** — the platform disallows custom lookups to `PricebookEntry` (*"referenceTo value of 'PricebookEntry' does not resolve to a valid sObject type"*, confirmed via deploy dry-run). **Implemented instead as two lookups: `Product2__c` + `Pricebook2__c`**, which together identify the same (product, segment-pricebook) coordinate a PricebookEntry represents. Segment-awareness preserved; a blank `Pricebook2__c` makes the tier global across segments (so the "global tiers" alternative survives as a per-record option, not an object-wide choice).
  - **Platform constraint also discovered**: `Product2` cannot be the parent of a cascade/restrict lookup, and the platform requires *required* lookups to declare cascade/restrict. Consequently **all lookups to `Product2` are not-required + SetNull** (`Customer_Price__c.Product2__c`, `Price_Tier__c.Product2__c`, `Stock_Reservation__c.Product2__c`); the "a price/reservation must reference a product" invariant is enforced by the Apex/Flow layer in Phase 2, not by the schema.
